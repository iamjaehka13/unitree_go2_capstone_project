...
"$PYTHON" project/scripts/render_clean_tripod_video.py \
  ...
  --checkpoint "$(checkpoint "$E4L_RUN")" \
  --policy-id E4L \
  --eval-init locked_tucked_clean \
  ...
  --device "$DEVICE" \
  --quiet \
  --output "$OUTPUT_DIR/E4L_locked_tucked_clean_no_push.mp4"

echo "[INFO] Saved $OUTPUT_DIR/E4L_locked_tucked_clean_no_push.mp4"
