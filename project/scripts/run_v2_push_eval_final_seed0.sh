...
OUTPUT="${REPO_ROOT}/project/results/push_eval/push_eval_v2_final_seed0.csv"
FIG_DIR="${REPO_ROOT}/project/results/figures/push_eval_v2_final_seed0"
...
echo "[INFO] v2 final push eval started at $(date --iso-8601=seconds)"
...
run_policy() {
  ...
  for direction in "${DIRECTIONS[@]}"; do
    for level in "${LEVELS[@]}"; do
      echo
      echo "[INFO] Evaluating policy=${policy_id} init=${eval_init} direction=${direction} level=${level}"
      "${PYTHON}" "${EVAL}" \
        --task "${task}" \
...
# E1 is included as a failure reference. It usually fails before the push.
...
echo
echo "[INFO] v2 final push eval finished at $(date --iso-8601=seconds)"
